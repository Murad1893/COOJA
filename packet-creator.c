...
/*----------------------------------------------------------------------------*/
void
create_and_send_ack(packet_t* p){
  if(address_cmp(&(p->header.src),&(conf.sink_address)) == 0){
    uint8_t i = 0;
    ...
    if (r != NULL){
      ...
      r->header.typ = ACK; //setting type to ack
      r->header.nxh = p->header.src;

      set_payload_at(r, 0, 1); //sending acknowledgement

      printf("\nSending ack from %d to %d\n", conf.my_address.u8[1], p->header.src.u8[1]);
      rf_unicast_send(r);
      rf_broadcast_send(r);
    }
  }
...
void create_and_send_data(void* ptr)
  {
    ...
    if(!neighbor_table_contains(&(conf.sink_address))){
      stat.data_packsent = 1;
    }
