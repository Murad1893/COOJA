...
/*----------------------------------------------------------------------------*/
  ...
  static void handle_ack(packet_t*);
...
/*----------------------------------------------------------------------------*/
  ...
  void
  handle_ack(packet_t* p){
    stat.ackrcv_time = clock_time();
    stat.ack_time = stat.ackrcv_time - stat.sent_time;
    stat.data_packsent = 0;
    PRINTF("Ack received from %d. Time taken: %lu milli seconds\n", p->header.src.u8[1], (1000 * stat.ack_time)/CLOCK_SECOND);
  }
...
/*----------------------------------------------------------------------------*/
  ...
  {
    if(((clock_time() - stat.sent_time)/CLOCK_SECOND) > stat.max_ack_time && stat.data_packsent == 1){
      PRINTF("\nAck not yet received. Time taken: %lu seconds\n", (clock_time() - stat.sent_time)/CLOCK_SECOND);
      stat.data_packsent = 0;
    }
...
/*----------------------------------------------------------------------------*/
  ...
  {
    #if !SINK
      create_and_send_ack(p); //sending ack for receiving packet
    #endif
    if (is_my_address(&(p->header.dst)))
    {
      PRINTF("\n[PHD]: Consuming Packet...\n");
#if SEND_DATA && SINK
